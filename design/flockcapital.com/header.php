<?php
	...
	$tab	= array_key_exists('tab', $_GET)&& !empty($_GET['tab']) ? $_GET['tab']	: null;
...
<html dir="ltr" lang="en">
    ...
    <body onload="" class="<?php echo $handle.' '.$tab; ?>">
    	
    	<div id="wrap">
        	<?php /*	TODO:
        				• Responsive height — taller on page load to support bigger logo pushing the brand.
        				• The menu should become collapsible when the viewport is too narrow to accommodate all menu items horizontally
        	*/ ?>
        	<header id="masthead" class="<?php echo in_array($handle, array('home')) ? ''  : 'mini'; ?>">
        		<div>
	        		<div id="logo">
	        			<div>
							<h1 itemprop="logo">
								<a href="?">
									<svg version="1.1" id="Layer_2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
										 width="100%" height="100%" viewBox="0 -6.001 2036.664 384.71" enable-background="new 0 -6.001 2036.664 384.71" xml:space="preserve">
					...
					<nav id="nav">
						<div><?php
							...
							?>
							<ul class="<?php echo $is_user_page ? 'focus-user-menu' : ''; ?>">
								 <li><a href="?p=projects" <?php if (array_key_exists('p', $_GET) && ($_GET['p']=='projects' || $_GET['p']=='project')) echo 'class="current"' ?>>Investments</a></li
								><li><a href="?p=managers" <?php if (array_key_exists('p', $_GET) && ($_GET['p']=='managers' || $_GET['p']=='manager')) echo 'class="current"' ?>>Managers</a></li
								><?php
									...
									if (isset($_SESSION['auth']) && $_SESSION['auth']) {
										...
										?><li><a href="?p=help" <?php if (array_key_exists('p', $_GET) && ($_GET['p']=='help')) echo 'class="current"' ?>>Help</a></li
										 ><li class="profile-badge <?php if ($is_user_page) echo 'current' ?>">
											...
											<ul id="user-menu">
												<li class="dashboard"><a href="?p=dashboard" <?php if (array_key_exists('p', $_GET) && ($_GET['p']=='dashboard')) echo 'class="current"' ?>>Dashboard</a></li>
												<li class="profile"><a href="?p=investor&i=<?php echo $_SESSION['user'] ?>" <?php if ((array_key_exists('p', $_GET) && ($_GET['p']=='investor' || $_GET['p']=='investor-edit')) && (array_key_exists('i', $_GET) && ($_GET['i']==$_SESSION['user']))) echo 'class="current"' ?>>Profile</a></li>
												<li class="settings"><a href="?p=settings&i=<?php echo $_SESSION['user'] ?>" <?php if (array_key_exists('p', $_GET) && ($_GET['p']=='settings')) echo 'class="current"' ?>>Settings</a></li>
												<li>
													<a href="#" onClick="document.logoutForm.submit()">Log Out</a>
													<form style="display:none;" method="post" name="logoutForm">
